...
void iforest::computeAnomalyScore(int pointX, const data & testDataObject){
	...
	long double avgNearestPointDistance;

	computeAvgPathLengthAndNearestPointDistance(pointX, testDataObject, avgPathLength, avgNearestPointDistance);
	//long double AscoreComputed = pow(2,-(avgPathLength/_avgPLComputationOfBST));       
	long double AscoreEstimated = pow(2,-(avgPathLength/_avgPLEstimationOfBST)) + 1.0 - pow(2, -(avgNearestPointDistance));
...
void iforest::computeAvgPathLengthAndNearestPointDistance(int pointX, const data & testDataObject, long double & avgPathLength, long double & avgNearestPointDistance)
{
	avgPathLength = 0.0;
	avgNearestPointDistance = 0.0;
	for(int treeId = 0; treeId < _numiTrees; treeId++){
	...
	avgNearestPointDistance /= _numiTrees;
}
