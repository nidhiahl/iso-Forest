...
class iforest
{
    public:
	//iforest();
    iforest(const data &, const data &);                              //used for: create an empty forest load the iforest from file and insert new points in data object
	iforest(const data &, int, int, double, int, const data &);                    //used for: create a forest using data object and initialize _sampleSize and _numiTrees
    virtual ~iforest();
