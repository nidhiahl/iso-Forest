...
long double iforest::avgPathLengthEstimationOfBST(int size){
	// I have increased places by 1 for accomodating 2 instead of 1 in the leaf nodes. Please check//
	long double avgPathLengthEstimationOfBST = 0;
	if(size == 3 || size == 4){avgPathLengthEstimationOfBST=1;}
	else if(size > 4){avgPathLengthEstimationOfBST = (2 * (log(size-4) + 0.5772156649)) - (2 * (size - 4) / size);}	
