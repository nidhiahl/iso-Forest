...
itree::itree(const data & dataObject, int sampleSize, int maxTreeHeight, int maxNumOfNodes, int avgPLEstimation, int powerForSegment): _dataObject(dataObject), _sampleSize(sampleSize), _maxTreeHeight(maxTreeHeight), _maxNumOfNodes(maxNumOfNodes), _powerForSegment(powerForSegment){}
    ...
    while(!BFTforNodes.empty()){
		...
		if(currNode){
			...
			if(currNode->nodeSize <=1 || currNode->nodeHeight ==_maxTreeHeight){
    			...
    			currNode->splitValue = currNode->splitInfoSelection(_dataObject, _powerForSegment);
